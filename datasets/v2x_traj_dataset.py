...
class V2XTrajDataset(Dataset):
    ...
    def __init__(self,
        ...
        super(V2XTrajDataset, self).__init__(root, transform=transform)

    @property
    def raw_dir(self) -> str:
        return os.path.join(self.root, 'v2x_test', self._directory, 'data')
    ...
    @property
    def veh_raw_dir(self) -> str:
        return os.path.join(self.root, 'vehicle-trajectories', self._directory, 'data')
    ...
    def processed_dir(self) -> str:
        return os.path.join(self.root, 'v2x_test', self._directory, 'processed')
    ...
    def process(self) -> None:
        ...
        for raw_path in tqdm(self.resume_paths):
            ...
            veh_path = os.path.join(self.veh_raw_dir, df_name)
            asso_path = os.path.join(self.traj_match_labels_dir, df_name)

            kwargs = process_argoverse(self._split, raw_path, dm, self._local_radius, road_path, veh_path, asso_path)
...
def get_traj_match_labels(asso_path, actor_ids):
    ...
    historical_timestamps = timestamps[: 40]
    ...
    for id in v2x_ids:
        ...
        ego_side_ids = []
        for iter in ego_id:
            if iter in actor_ids:
                ego_side_ids.append(actor_ids.index(iter))
        coop_side_ids = []
        for iter in coop_id:
            if iter in actor_ids:
                coop_side_ids.append(actor_ids.index(iter))
...
def process_argoverse(split: str,
    ...
    v2x_df = pd.concat([ego_df, road_df, veh_df], ignore_index=True)
    ...
    veh_historical_df = veh_df[veh_df['timestamp'].isin(historical_timestamps)]
    veh_df_id = veh_historical_df.id.drop_duplicates(inplace=False)
    
    v2x_historical_df = v2x_df[v2x_df['timestamp'].isin(historical_timestamps)]
    actor_ids = list(v2x_historical_df['id'].unique())
    ...
    origin = torch.tensor([av_df[39]['x'], av_df[39]['y']], dtype=torch.float64)
    av_heading_vector = origin - torch.tensor([av_df[38]['x'], av_df[38]['y']], dtype=torch.float)
    theta = torch.atan2(av_heading_vector[1], av_heading_vector[0])
    rotate_mat = torch.tensor([[torch.cos(theta), -torch.sin(theta)],
                               [torch.sin(theta), torch.cos(theta)]], dtype=torch.float64)
    # initialization
    x = torch.zeros(num_nodes, 80, 2, dtype=torch.float64)
    last_positions = torch.zeros(num_nodes, 2, dtype=torch.float64)
    edge_index = torch.LongTensor(list(permutations(range(num_nodes), 2))).t().contiguous()
    padding_mask = torch.ones(num_nodes, 80, dtype=torch.bool) # False means valid
    bos_mask = torch.zeros(num_nodes, 40, dtype=torch.bool)
    ...
    veh_mask = torch.zeros(num_nodes, dtype=torch.bool)

    ego_mask[:len(ego_df_id)] = True
    road_mask[len(ego_df_id):len(ego_df_id)+len(road_df_id)] = True
    veh_mask[len(ego_df_id)+len(road_df_id):] = True
    ...
    for actor_id, actor_df in v2x_df.groupby('id'):
        ...
        node_historical_steps = list(filter(lambda node_step: node_step < 40, node_steps))
        ...
        min_box_ls[node_idx] = cv2.boxPoints(cv2.minAreaRect(np.float32(x[node_idx, :40, :][~padding_mask[node_idx, :40]])))

    # bos_mask is True if time step t is valid and time step t-1 is invalid
    bos_mask[:, 0] = ~padding_mask[:, 0]
    bos_mask[:, 1: 40] = padding_mask[:, : 39] & ~padding_mask[:, 1: 40]

    positions = x.clone()
    
    x[:, 40:] = x[:, 40:] - x[:, 39].unsqueeze(-2) #vectorize?
    x[:, 1: 40] = torch.where((padding_mask[:, : 39] | padding_mask[:, 1: 40]).unsqueeze(-1),
                              torch.zeros(num_nodes, 39, 2),
                              x[:, 1: 40] - x[:, : 39]) # difference
    ...
    y = None if split == 'test' else x[:, 40:]

    ego_ids = range(len(ego_df_id))
    road_ids = range(len(ego_df_id), len(ego_df_id)+len(road_df_id))
    veh_ids = range(len(ego_df_id)+len(road_df_id), num_nodes)

    v2x_all_index = list(product(ego_ids, road_ids))
    v2x_all_index.extend(list(product(road_ids, ego_ids)))
    v2x_all_index.extend(list(product(ego_ids, veh_ids)))
    v2x_all_index.extend(list(product(veh_ids, ego_ids)))
    v2x_all_index.extend(list(product(road_ids, veh_ids)))
    v2x_all_index.extend(list(product(veh_ids, road_ids)))
    ...
    for idx, pair in enumerate(v2x_all_index):
        ...
        if actor_type[pair[0]] == actor_type[pair[1]]:
            ...
            continue
        aa_iou = aabox_overlap(x[pair[0], :40, :][~padding_mask[pair[0], :40]],
                      x[pair[1], :40, :][~padding_mask[pair[1], :40]])
        if aa_iou > 0:
    ...
    edge_road = torch.LongTensor(list(permutations(range(len(ego_df_id), len(ego_df_id)+len(road_df_id)), 2))).t().contiguous()
    edge_veh = torch.LongTensor(list(permutations(range(len(ego_df_id)+len(road_df_id), num_nodes), 2))).t().contiguous()
    ...
    if len(edge_veh.shape) > 1:
        interact_veh_mask = rowwise_in(edge_index.t(), edge_veh.t())
    else:
        interact_veh_mask = torch.zeros(len(edge_index.t()), dtype=torch.bool).t().contiguous()
    ...
    return {
        ...
        'interact_veh_mask': interact_veh_mask,
        'actor_type': actor_type,
        'y': y.float(),  # [N, 50, 2]
        'num_nodes': num_nodes,
        'num_ego_actors': len(ego_df_id),
        'num_road_actors': len(road_df_id),
        'num_veh_actors': len(veh_df_id),
        'ego_mask': ego_mask,
        'road_mask': road_mask,
        'veh_mask': veh_mask,
        ...
        'theta': theta.float(),
        'last_positions': last_positions.float(),
    }
