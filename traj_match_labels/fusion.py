...
class Fusion(object):
    ...
    def __init__(self,iou_threshold = 0.3, iou_threshold_3d = 0.01, hungarian=True, dist_flag = 'iou_2d',is_solve_wrong_association=True, min_asso_ratio=0.20):
        self.logger = logger
        ...
        self.min_asso_ratio = min_asso_ratio
        ...
        for track_id, v2i_value in track_fusion_info.items():
            ...
            if matched_id_nums == 1:
                ...
                if len(matched_id_list_of_matched_id) == 1:
                    ...
                    if (association_pts_nums < MIN_ASSO_PTS_NUMS or association_ratio < self.min_asso_ratio) and (association_pts_nums_inv < MIN_ASSO_PTS_NUMS or association_ratio_inv  < self.min_asso_ratio):
                        #can not
                        if is_reverse:
                    ...
                    if association_pts_nums < MIN_ASSO_PTS_NUMS or association_ratio < self.min_asso_ratio:
                        #can not
                        if is_reverse:
                            cannot_fusion_v2i.add((matched_id,track_id))
                        else:                            
